...
def knnm(train_data, test_data, labels, k):
    """
    implement of knn-means
    ...
    while i < row_size:
        ...
        if sum(num) == 10 * k:
            break
        elif num.get(vote, 0) == k:
            i += 1
        else:
            counts[vote] = counts.get(vote, 0) + distances[sort_distance[i]]
            num[vote] = counts.get(vote, 0) + 1
            i += 1
...
if __name__ == "__main__":
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        for index in range(X_test.shape[0]):
            predict = knnm(X_train, X_test[index, :], y_train, k=42)
